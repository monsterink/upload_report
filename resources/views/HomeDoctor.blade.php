...
<div class="mt-4">
  ...
  <table class="table">
    ...
    @if (count($uploadfiles) !=0  && Auth::user()->role!=1)
      @foreach ($uploadfiles as $key => $uploadfile) 
        <tbody>
          <tr>
            ...
            <td>{{$uploadfile->status}}
            @if ($uploadfile->status=='Deleted')
            ...
            @endif
              ...
              @if ($uploadfile->status!='Deleted' && Auth::user()->role==3)
              ...
              @endif
            @if ($uploadfile->status=='Uploaded' && Auth::user()->role==2)
              <td>
                  <a class="btn btn-warning" href="{{url('/uploads/edit/'.$uploadfile->id)}}" role="button">Edit</a>
                  <a class="btn btn-danger" href="{{url('/delete/'.$uploadfile->id)}}" role="button">Delete</a>
              </td>
              @endif
          </tr>
        </tbody>
